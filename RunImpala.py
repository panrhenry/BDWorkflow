...
DB_CONF = None
# 配置文件
CONF_PATH = sys.path[0] + "/conf/RunImpala.conf"
CONF = None
MYSQL = None
# 流水号
LSH = None
...
def init_param():
    ...
    try:
        # 获取命令行参数
        opts, args = getopt.getopt(sys.argv[1:], "ht:v:l:", ["help", "type=", "var=", "lsh=", "host_id=", "file="])
        if len(opts) == 0:
            show_help()
...
def getRunCmd(proc):
    ...
    if run_bin.count("%s") == 1:
        run_bin = run_bin % HOSTNODE["HOST_IP"]
    elif run_bin.count("%s") == 2:
        run_bin = run_bin % (HOSTNODE["HOST_IP"], HOSTNODE["USER_NAME"])
    elif run_bin.count("%s") == 3:
        run_bin = run_bin % (HOSTNODE["HOST_IP"], HOSTNODE["USER_NAME"], des(key=DB_CONF.get("conf", "des_key"), padmode=PAD_PKCS5).decrypt(base64.b64decode(HOSTNODE["PASSWORD"])).decode('utf-8', errors='ignore'))

    param_str = ""
    for item in VAR:
        param_str += "--var=%s=%s " % (item, VAR[item])
    cmd = '%s -f %s %s' % (run_bin, WORKPATH + "/" + proc, param_str)
    LOGGER.info(cmd)
    return cmd
...
def main():
    try:
        ...
        global WORKPATH
        WORKPATH = sys.path[0]
        definitions = json_sql["impala_files"]
